...
%
综上，我们将全连接神经网络的推断和训练在\autoref{alg:ss-perm:train,alg:ss-perm:infer}中进行描述。
%
\begin{algorithm}[h!]
    ...
    \label{alg:ss-perm:train}
        \begin{algorithmic}[1]
        \Require{秘密分享的输入$\langle X \rangle$, 秘密分享的第$i$层权重$\langle W \rangle_i$，偏置$\langle \bvec b \rangle_i$, 激活函数$f_i$, 总层数$L$}
...
\label{alg:ss-perm:infer}
    ...
    \For{$i = L - 1$ to $0$} \LineComment{$L$为总层数}
        \State $G_{i + 1} \gets \mathsf{PermNonlinear}(Z_{i+1}, \dfrac{df_i}{dx})$ \LineComment{$\dfrac{df_i}{dx}$也是逐元素函数}
        \State $G_{i + 1} = \mathsf{SSMul}(\langle G_{i+1} \rangle, \langle W_i^T \rangle, \text{矩阵乘})$ \LineComment{对权重进行了转置}
